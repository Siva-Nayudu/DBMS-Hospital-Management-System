...
class Appointment
{
	...
	{
		if ($this->id) {
			$sqlQuery = "SELECT a.id, d.id as d_id, d.name as doctor_name, s.id as s_id, s.specialization, a.consultancy_fee, appointment_date, a.appointment_time, a.created, a.status, p.name as patient_name, p.id as patient_id
			FROM " . $this->appointmentTable . " a 
			LEFT JOIN " . $this->doctorTable . " d ON a.doctor_id = d.id
			LEFT JOIN " . $this->patientsTable . " p ON a.patient_id = p.id
